...
const toMachine = (row: Record<string, any>) => ({
  ...
  tarifRotation:
    row.tarifRotation == null ? undefined : asNumber(row.tarifRotation),
  tarifDegressif:
    row.tarifDegressif == null ? undefined : asNumber(row.tarifDegressif),
...
export const catalogueService = {
  async getAdminMachines() {
    return (await catalogueRepository.listAllMachines()).map(row =>
      toMachine(row as Record<string, any>)
    );
  },

  async getPublishedMachines() {
    return (await catalogueRepository.listPublishedMachines()).map(row =>
      toMachine(row as Record<string, any>)
    );
  },
  ...
  async getAdminPieces() {
    return (await catalogueRepository.listAllPieces()).map(row =>
      toPiece(row as Record<string, any>)
    );
  },

  async getPublishedPieces() {
    return (await catalogueRepository.listPublishedPieces()).map(row =>
      toPiece(row as Record<string, any>)
    );
  },
  ...
  async saveMachine(input: CatalogueMachineInput) {
    ...
    return toMachine(
      (await catalogueRepository.upsertMachine({
        ...rest,
  ...
  async savePiece(
    input: Parameters<typeof catalogueRepository.upsertPiece>[0]
  ) {
    return toPiece(
      (await catalogueRepository.upsertPiece(input)) as Record<string, any>
    );
  },
};
